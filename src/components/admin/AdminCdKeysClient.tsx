...
export default function AdminCdKeysClient({ initial }: { initial?: Data }) {
  ...
  return <AdminPage>
    ...
    {data.summary.available <= 5 ? <AdminCard className="mb-3 border-amber-400/20 bg-amber-400/[.05] p-3">
      <div className="text-xs font-black text-amber-200">موجودی CD Key پایین است.</div>
      <div className="mt-1 text-[10px] leading-5 text-brand-m_khonsa">فقط {Number(data.summary.available).toLocaleString("fa-IR")} کلید آماده تخصیص باقی مانده؛ قبل از رسیدن سفارش‌های بعدی موجودی را بررسی کن.</div>
    </AdminCard> : null}
    ...
    <AdminCard className="overflow-hidden">
      ...
      <div className="lg:hidden space-y-2 p-3">{data.nodes.map(row=><div key={row.stockId} className="rounded-[5px] border border-white/[.06] bg-black/10 p-3"><div className="flex items-start justify-between gap-3"><div><div className="text-xs font-black text-white">{row.productName}</div><div className="mt-1 text-[9px] text-brand-m_khonsa">#{row.stockId} · {row.variationName}</div></div><AdminBadge tone={row.status==="available"?"success":row.status==="decrypt_failed"?"danger":"neutral"}>{LABELS[row.status]??row.status}</AdminBadge></div><div className="mt-3 grid grid-cols-2 gap-3"><div><div className="text-[9px] text-brand-m_khonsa">سفارش</div><div className="mt-1 text-[10px] text-white" dir="ltr">{row.orderId?`#${row.orderId}`:"—"}</div></div><div><div className="text-[9px] text-brand-m_khonsa">تلاش تخصیص</div><div className="mt-1 text-[10px] text-white" dir="ltr">{row.assignmentAttempts}</div></div></div>{permissions.includes("cdkeys.write") && ["available","duplicate","failed","decrypt_failed"].includes(row.status) && !row.orderId && <div className="mt-3 flex justify-end"><AdminButton variant="danger" onClick={()=>void deleteKey(row.stockId)} disabled={loading}>حذف کلید</AdminButton></div>}</div>)}{data.nodes.length===0&&<AdminEmpty title="کلیدی با این فیلتر پیدا نشد."/>}</div>
