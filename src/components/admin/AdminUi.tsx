...
export function AdminBackLink({ href, children }: { href: string; children: ReactNode }) {
  return <Link href={href} prefetch={false} className="inline-flex items-center gap-1.5 text-sm font-medium text-brand-m_khonsa transition-colors hover:text-white">{children}</Link>;
}
...
export function AdminSectionHeader({ icon, title, description, action }: { icon?: ReactNode; title: string; description?: string; action?: ReactNode }) {
  return (
    <div className="flex flex-col gap-2 border-b border-white/[.06] px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
...
export function AdminFilterBar({ children, className = "" }: { children: ReactNode; className?: string }) {
  return <div className={`mb-4 rounded-[4px] border border-brand-surface_hover bg-brand-surface p-3 ${className}`}><div className="flex flex-col gap-2 lg:flex-row lg:items-center">{children}</div></div>;
}
...
export function AdminListCard({ children, href, className = "" }: { children: ReactNode; href?: string; className?: string }) {
  const classes = `block rounded-[4px] border border-brand-surface_hover bg-brand-surface p-4 transition-colors hover:border-brand-blue/20 hover:bg-white/[.018] ${className}`;
