...
export default function AdminOrderDetail({ order: initialOrder, id }: { order?: Order; id: number }) {
  ...
  return <AdminPage>
    ...
    <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1.35fr_0.75fr]">
      ...
      <div className="space-y-4"><AdminCard className="overflow-hidden"><AdminSectionHeader title="مشتری"/><div className="p-4"><AdminFieldRow label="نام" value={order.customerName || "—"} ltr={false}/><AdminFieldRow label="ایمیل" value={order.customerEmail || "—"}/></div></AdminCard>
