  ...
  return (
    <div className="h-[100dvh] w-full bg-brand-bg flex flex-col overflow-hidden" dir="rtl">
      <header className="w-full flex items-center justify-between px-5 py-4 md:px-8 shrink-0">
        <div className="h-9 w-[106px] bg-white/[.03] rounded" />
        <div className="h-4 w-24 bg-white/[.03] rounded" />
      </header>
      ...
      <main className="flex-1 min-h-0 w-full flex items-center justify-center p-4 md:p-5 overflow-y-auto pb-[calc(58px+env(safe-area-inset-bottom)+12px)] lg:pb-4">
        <section className="w-full max-w-md bg-brand-surface border border-brand-surface_hover p-6 md:p-8 flex flex-col gap-6">
          <div className="flex flex-col items-center gap-3 pb-5 border-b border-brand-surface_hover">
            <div className="h-9 w-[130px] bg-white/[.03] rounded" />
            <div className="h-4 w-28 bg-white/[.03] rounded" />
          </div>

          <div className="flex flex-col items-center gap-2">
            <div className="h-7 w-40 bg-white/[.03] rounded" />
            <div className="h-3.5 w-[92%] max-w-[300px] bg-white/[.03] rounded" />
            <div className="h-3.5 w-[72%] max-w-[230px] bg-white/[.03] rounded" />
          </div>

          <div className="grid grid-cols-2 gap-1.5 bg-brand-bg p-1 border border-brand-surface_hover">
            <div className="h-10 w-full bg-white/[.03] rounded" />
            <div className="h-10 w-full bg-white/[.03] rounded" />
          </div>

          <div className="flex flex-col gap-5">
            <div className="flex flex-col gap-1.5">
              <div className="h-3 w-24 bg-white/[.03] rounded" />
              <div className="h-[48px] w-full bg-white/[.03] rounded" />
            </div>
            <div className="h-[48px] w-full bg-white/[.03] rounded" />
            <div className="h-3 w-32 self-center bg-white/[.03] rounded" />
          </div>
        </section>
      </main>
