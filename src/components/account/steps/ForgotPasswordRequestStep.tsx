...
export default function ForgotPasswordRequestStep({
  onBack,
  onCodeSent,
}: {
  onBack: () => void;
  onCodeSent: (identifier: string, channel: string, cooldownSeconds: number) => void;
}) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data?.error || "ارسال کد بازیابی با خطا مواجه شد");
        return;
      }

      onCodeSent(trimmed, data.channel ?? "sms", data.cooldownSeconds ?? 60);
    } catch {
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="relative">
          <KeyRound size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-brand-surface_m" />
          <input
            type="text"
            value={identifier}
